...
public:
   ...
   virtual ~CXSocket() { Disconnect(); }

   void SetTarget(string ip, int port, int timeout_ms = 3000) { m_ip = ip; m_port = port; m_timeout_ms = timeout_ms; }
   ENUM_SOCKET_STATE GetState() const { return m_state; }
   bool IsConnected() { return (m_state == STATE_CONNECTED && m_handle != INVALID_HANDLE); }
   ...
   void Update() {
      ...
      if(m_state == STATE_DISCONNECTED && m_ip != "") {
         if(now >= m_next_reconnect_time) Connect(m_ip, m_port, m_timeout_ms);
      }
   }

   bool Connect(string ip, int port, int timeout_ms = 3000) {
      m_ip = ip; m_port = port; m_timeout_ms = timeout_ms;
      this.Disconnect();
      m_handle = SocketCreate(SOCKET_DEFAULT);
      if(m_handle == INVALID_HANDLE) { SetNextReconnectTime(); return false; }
      m_state = STATE_CONNECTING;
      if(!SocketConnect(m_handle, m_ip, m_port, m_timeout_ms)) { Disconnect(); SetNextReconnectTime(); return false; }
   ...
   string Receive_Json() {
      ...
      if(lineLen <= 2) return (m_recv_buffer != "") ? this.Receive_Json() : "";
      ...
      return (m_recv_buffer != "") ? this.Receive_Json() : "";
   }
   ...
   string Receive_CSV() {
      ...
      if(len > 0) {
         ...
         } else if(rev < 0) { Disconnect(); return ""; }
      }
